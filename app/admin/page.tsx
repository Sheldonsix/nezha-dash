...
function adminCookieValue() {
  ...
  return createHmac("sha256", password).update("nezha-dash-admin").digest("hex")
}
    ...
    return (
      <main className="mx-auto grid w-full max-w-5xl gap-4 bg-background p-4 md:gap-6 md:p-10 md:pt-8">
        ...
        <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <StatCard icon={<Server className="size-4" />} label="节点总数" value={rows.length} />
          <StatCard icon={<Wifi className="size-4" />} label="在线" value={onlineCount ?? "-"} />
          <StatCard
        ...
        <Card>
          ...
          <CardContent>
            <form action={createServerAction} className="grid gap-3 md:grid-cols-3">
                    ...
                    <Td>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-muted-foreground text-xs">{server.username}</div>
                        </div>
                      </div>
                    </Td>
                    <Td>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {server.disabled && <Badge variant="secondary">已禁用</Badge>}
                      </div>
                    </Td>
                    <Td>
    ...
    >
      <section className="flex flex-col items-start gap-2">
        <label className="flex flex-col items-start gap-1">
          {error && <p className="font-semibold text-red-500 text-sm">{t("ErrorMessage")}</p>}
    ...
    <main className="mx-auto w-full max-w-5xl">
      <Card>
        <CardContent className="space-y-3 p-6">
          ...
          <p className="text-muted-foreground text-sm">缺少环境变量：{missing.join(", ")}</p>
        </CardContent>
      </Card>
    </main>
  )
...
function EventRow({ event }: { event: NodeStatusAdminEvent }) {
  return (
    <tr className="border-b last:border-0">
      ...
      <Td>{event.resolved ? <Badge variant="secondary">已恢复</Badge> : <Badge>未恢复</Badge>}</Td>
