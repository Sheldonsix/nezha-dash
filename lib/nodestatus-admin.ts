...
async function adminToken() {
  ...
  const username = getEnv("NodeStatusWebUsername") || "admin"
...
async function adminFetch<T>(path: string, init?: RequestInit, retry = true): Promise<T> {
...
export function getNodeStatusSnapshot() {
  return fetchWithRetry(`${baseUrl()}/api/status`, { cache: "no-store" }).then(async (res) => {
...
export function updateNodeStatusAdminServer(
  username: string,
  data: Partial<Omit<NodeStatusAdminServer, "id" | "order">> & { password?: string },
) {
  return adminFetch<null>("/api/admin/servers", {
    method: "PUT",
...
export function deleteNodeStatusAdminServer(username: string) {
  return adminFetch<null>(`/api/admin/servers/${encodeURIComponent(username)}`, {
    method: "DELETE",
  })
}
